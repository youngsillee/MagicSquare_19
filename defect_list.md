...
| **작성일** | 2026-05-29 |
| **최종 회귀** | `pytest tests/` — **53 passed** (2026-05-29) |
...
| DEF-004 | Medium | AC-FR-01-01 | `python -m pytest tests/ -m ac_fr_01_01 --cov=entity/rules --cov-report=term-missing` | Domain Logic(`entity/rules/`) 커버리지 **≥ 95%** | **~82%** (`grid_shape_validator` 유효 4×4 분기·`magic_square_judge.resolve` 본문 미실행) | AC-FR-01-01에 4×4 **정상 격자** 테스트·`resolve()` 성공 경로 없음 (범위 제한) | AC-FR-01-02+ 또는 REFACTOR에서 유효 격자 경로 테스트·구현 추가 — **미해결** |
...
| DEF-006 | Low | AC-FR-01-01 | `python -m pytest tests/ --cov=entity --cov=control --cov=boundary` | 전체 TOTAL 커버리지 **≥ 90%** | **89%** (53 passed) | `entity/models/user.py`·rules 미커버 분기·`judge_use_case` 27행(`resolve` 호출) 미실행 | 후속 AC 테스트 확대 또는 `--cov` 범위를 AC-FR-01-01 모듈로 한정 — **미해결** |
